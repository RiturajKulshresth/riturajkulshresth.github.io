    ...
    >
      <nav className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
        <Link
          href={brandHref}
          className={clsx(
            "group mr-6 flex shrink-0 items-center gap-2.5 text-sm font-medium tracking-tight lg:mr-10",
            BRAND
          )}
        >
          <span className="relative inline-flex h-2 w-2 rounded-full bg-[color:var(--color-success)] pulse-dot" />
          <span className="brand-shimmer whitespace-nowrap">Rituraj Kulshresth</span>
        </Link>
          ...
          <ul className="flex items-center gap-0.5 text-sm">
            ...
            <li ref={modesRef} className="relative">
              <button
                ...
                aria-haspopup="menu"
                className={clsx(
                  "nav-attention flex items-center gap-1 whitespace-nowrap rounded-md px-3 py-1.5 text-sm transition",
                  modesOpen || isRenderModeActive ? NAV_LINK_ACTIVE : NAV_LINK
                )}
              >
                <span className="nav-attention-label whitespace-nowrap font-semibold">Render Modes</span>
                <ChevronDown
                  className={clsx(
        ...
        <div className={clsx("border-t px-6 py-5 md:hidden", MOBILE_SURFACE)}>
          ...
          <div className="mt-4 flex flex-col gap-2">
            <a
              ...
              rel="noopener noreferrer"
              className={clsx(
                "rounded-md px-3 py-2 text-center text-sm transition",
                BUTTON_ACCENT
              )}
            >
              Résumé
